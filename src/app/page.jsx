...
export default function HomePage() {
  ...
  const renderLiveAd = (slotId) => {
    const matchingAds = (homepageAds || []).filter(a => slotMatchesAd(a, slotId));
    if (matchingAds.length === 0) return null;

    return (
      <div key={`live-ad-zone-${slotId}`} className="live-ad-dropzone-wrapper" style={{ width: '100%', boxSizing: 'border-box' }}>
        {matchingAds.map((ad, adIdx) => {
          ...
          // 1. FLOATED EDITORIAL AD CARD FORMAT (EXACT MATCH TO ADMIN STUDIO & ROLEX CHRONOMETER SCREENSHOT)
          if (isFloated) {
            return (
              <div 
